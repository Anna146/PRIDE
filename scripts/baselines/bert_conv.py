...
# Input files
train_files_path = project_dir + "/data/bert_conv_input/" + str(args.fold_num) + "/train/"
train_files = [os.path.join(train_files_path, f) for f in os.listdir(train_files_path)]
test_file = project_dir + "/data/bert_conv_input/" + str(args.fold_num) + "/test.txt"
dev_file = project_dir + "/data/bert_conv_input/" + str(args.fold_num) + "/dev.txt"
...
def train():
    ...
    global net
    config = BertConfig()
    net = BertForSequenceClassification(config).from_pretrained('bert-base-uncased', num_labels=len(predicate_list)).to(
        device)
    net.train()

    criterion = nn.BCEWithLogitsLoss()
    optimizer = AdamW(net.parameters(), lr=main_config.lr)
    streams = [
        pescador.Streamer(feature_gen_ham, ff, 1, ["input_ids", "label_ids", "segment_ids", "input_mask", "guid"])
        ...
        for output_file, streaming_file in zip([dev_output_file, test_output_file], [dev_file, test_file]):
            ...
            with open(output_file, "w") as f_test_out:
                ...
                for features in streamer:
                    features = features[0]
                    curr_mask = torch.tensor(features["input_mask"][0], dtype=torch.long).to(device)
                    curr_words = torch.tensor(features["input_ids"][0], dtype=torch.long).to(device)
                    curr_segments = torch.tensor(features["segment_ids"][0], dtype=torch.long).to(device)

                    one_logit = net(curr_words.unsqueeze(0), token_type_ids=curr_segments.unsqueeze(0),
                                    attention_mask=curr_mask.unsqueeze(0))[0]

                    for log in one_logit.cpu().data.numpy():
                        by_guid[features["guid"]].append(log)
                        by_guid_answ[features["guid"]] = features["label_ids"]

                for guid, utts in by_guid.items():
                    if main_config.chunk_agg == "avg":
                        utts = np.mean(np.array(utts), axis=0)
                    elif main_config.chunk_agg == "max":
                        utts = np.max(np.array(utts), axis=0)
